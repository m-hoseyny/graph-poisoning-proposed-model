...
def process_regressor(batch, gnn_model, edge_classifier_model, device, optimizer, criterion):
    num_nodes_all = [graph.x.shape[0] for graph in batch]
    node_features_all = [graph.x for graph in batch]
    node_feature_batch = torch.cat(node_features_all, dim=0).to(device)
    ...
    edge_labels_batch = edge_labels_batch.to(dtype=torch.float32)
    # Compute loss
    loss = criterion(out, edge_labels_batch)
...
def eval_process_regressor(batch, gnn_model, edge_classifier_model, device, optimizer, criterion):
    with torch.no_grad():
        num_nodes_all = [graph.x.shape[0] for graph in batch]
        node_features_all = [graph.x for graph in batch]
        node_feature_batch = torch.cat(node_features_all, dim=0).to(device)
